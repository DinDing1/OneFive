...
export const filesApi = {
  /** 列出目录内容 */
  listFiles(cid: string = '0', limit = 100, offset = 0, order = 'file_name', asc = 1): Promise<any> {
  ...
  /** 创建目录 */
  createFolder(name: string, pid: string = '0'): Promise<any> {
    return api.post('/files/mkdir', { name, pid: Number(pid) })
  },

  /** 搜索文件 */
  searchFiles(keyword: string, cid: string = '0'): Promise<any> {
    return api.get('/files/search', { params: { keyword, cid } })
  },

  /** 批量移动文件 */
  moveFiles(fileIds: string[], toCid: string): Promise<any> {
    return api.post('/files/move', { file_ids: fileIds, to_cid: toCid })
  },

  /** 批量复制文件 */
  copyFiles(fileIds: string[], toCid: string): Promise<any> {
  ...
  /** 获取 Open API 设置 */
  getOpenApiSettings(): Promise<any> {
    return api.get('/config/open-api')
  },

  /** 更新 Open API 设置 */
  updateOpenApiSettings(enabled: boolean, appId: string): Promise<any> {
